...
op {_|_} : Substitution Term -> SubstTerm .
op  nil  : -> SubstTermList .
op _::_  : SubstTermList SubstTermList -> SubstTermList [assoc id: nil] .
...
var  FRESH          : Term .
var  OBJECTFACTS    : FactSet .
var  FRESHCNT       : Term .
var  METAFACTS      : Term .
var  EXISTSUBST     : SubstTermList .
var  SPLITSUBST     : SubstTermList .
var  OK IHAVEIT     : Bool .
...
ceq splitMatch( METAFACTS ,  { SUB1 | '_;_[ T1, TL:TermList ] }  ) =
    if( not IHAVEIT ) then
        splitMatch(METAFACTS, { SUB1 | '_;_[ TL:TermList ] } )
    else
        true  
    fi
    if IHAVEIT := existMatch(METAFACTS, { SUB1 | T1 })
-----     [ print "[ splitMatch ]  IHAVEIT " IHAVEIT ] 
.

eq  splitMatch( METAFACTS,  { SUB1 | '_;_[ empty ] }  ) = false
-----     [ print "[ splitMatch ]  '_;_[ empty ] " ] 
.
...
---op isValid    : Search -> Bool .

vars RESULT CURMAX METAFACTS NEWMAX : Term .
vars PATHS                          : Branches .
vars BRANCH                         : Branch .
vars EXISTSUBST SPLITSUBST          : SubstTermList .
vars ONESUBST TWOSUBST              : SubstTermList .
vars SUB                            : Substitution .
vars T1 T2 T3 T4                    : Term .
vars PRESENT                        : Bool .
vars OBJECTFACTS                    : FactSet .


crl[ones]:
    < METAFACTS | CURMAX | nil | TWOSUBST | EXISTSUBST | SPLITSUBST >  stack  PATHS
=>
    < METAFACTS | CURMAX | ONESUBST | TWOSUBST | EXISTSUBST | SPLITSUBST >  stack  PATHS
if ONESUBST := generateSingles( singleRules, METAFACTS )
----- /\ OBJECTFACTS := dt(METAFACTS) 
----- [ print " ones:    " OBJECTFACTS ]
.

crl[ones]:
    < METAFACTS | CURMAX | { SUB | T1 } :: ONESUBST | TWOSUBST | EXISTSUBST | SPLITSUBST >  stack PATHS
=>
    < RESULT | CURMAX | ONESUBST | TWOSUBST | EXISTSUBST | SPLITSUBST >  stack PATHS
if RESULT  :=  join(METAFACTS, applySubst( { SUB | T1 } ))
----- /\ OBJECTFACTS := dt(RESULT) 
----- [ print " ones2:    " OBJECTFACTS ]
.

crl[twos]:
    < METAFACTS | CURMAX | ONESUBST | nil | EXISTSUBST | SPLITSUBST >  stack  PATHS
=>
    < METAFACTS | CURMAX | ONESUBST | TWOSUBST | EXISTSUBST | SPLITSUBST >  stack  PATHS
if TWOSUBST := generateDoubles( doubleRules, METAFACTS ) 
----- /\ OBJECTFACTS := dt(METAFACTS)
----- [ print " twos:    " OBJECTFACTS ]
.

crl[twos]:
    < METAFACTS | CURMAX | ONESUBST | { SUB | T1 } :: TWOSUBST | EXISTSUBST | SPLITSUBST >  stack PATHS
=>
    < RESULT | CURMAX | ONESUBST | TWOSUBST | EXISTSUBST | SPLITSUBST >  stack PATHS
if RESULT  :=  join(METAFACTS, applySubst( { SUB | T1 } ))
----- /\ OBJECTFACTS := dt(RESULT) 
----- [ print " twos2:   " OBJECTFACTS ]
.

crl[exists]:
    < METAFACTS | CURMAX | ONESUBST | TWOSUBST | nil | SPLITSUBST >  stack PATHS
=>
    < METAFACTS | CURMAX | ONESUBST | TWOSUBST | EXISTSUBST | SPLITSUBST >  stack PATHS
if EXISTSUBST := generateExists(existRules, METAFACTS) 
----- /\ OBJECTFACTS := dt(METAFACTS) 
----- [ print " exists1: " OBJECTFACTS ]
.

crl[exists]:
    < METAFACTS | CURMAX | ONESUBST | TWOSUBST | { SUB | T1 } :: EXISTSUBST | SPLITSUBST >  stack PATHS
=>
    < RESULT | NEWMAX | ONESUBST | TWOSUBST | EXISTSUBST | SPLITSUBST >  stack PATHS
if RESULT  :=  join(METAFACTS, applySubst( refreshSubst( CURMAX, { SUB | T1 } )))
/\ NEWMAX  :=  getTerm( metaReduce(geotheory, 'maximum[ RESULT ] ) )
/\ PRESENT :=  existMatch( METAFACTS, { SUB | T1 } )
/\ not PRESENT 
----- /\ OBJECTFACTS := dt(RESULT) 
----- [ print " exists2: " OBJECTFACTS  " | PRESENT " PRESENT ]
.
...
=>
    < METAFACTS | CURMAX | ONESUBST | TWOSUBST | EXISTSUBST | SPLITSUBST >  stack PATHS
if PRESENT :=  existMatch( METAFACTS, { SUB | T1 } )
/\ PRESENT 
----- /\ OBJECTFACTS := dt(METAFACTS) 
----- [ print " exists3: " OBJECTFACTS " | PRESENT " PRESENT ]
.

crl[splits]:
    < METAFACTS | CURMAX | ONESUBST | TWOSUBST | EXISTSUBST | nil >  stack PATHS
=>
    < METAFACTS | CURMAX | ONESUBST | TWOSUBST | EXISTSUBST | SPLITSUBST >  stack PATHS
if SPLITSUBST := generateSplitts(splitRules, METAFACTS) 
----- /\ OBJECTFACTS := dt(METAFACTS)
----- [ print " splits1: " OBJECTFACTS ]
.

crl[splits]:
    < METAFACTS | CURMAX | ONESUBST | TWOSUBST | EXISTSUBST | 
    { SUB | '_;_[ T1, NE:NeTermList ] } :: SPLITSUBST >  
    stack PATHS
=>
    < RESULT | NEWMAX | ONESUBST | TWOSUBST | EXISTSUBST | SPLITSUBST >  
    stack PATHS
    < METAFACTS | CURMAX | ONESUBST | TWOSUBST | EXISTSUBST | 
    { SUB | '_;_[ NE:NeTermList ] } :: SPLITSUBST >
if RESULT  :=  join(METAFACTS, applySubst( refreshSubst( CURMAX, { SUB | T1 } ))) 
/\ NEWMAX  :=  getTerm( metaReduce(geotheory, 'maximum[ RESULT ] ))
/\ PRESENT :=  splitMatch(METAFACTS, { SUB | '_;_[ T1, NE:NeTermList ] })
/\ not PRESENT 
----- /\ OBJECTFACTS := dt(RESULT)
----- [ print " splits2: " OBJECTFACTS " | PRESENT " PRESENT ]
.
...
=>
    < METAFACTS | CURMAX | ONESUBST | TWOSUBST | EXISTSUBST | SPLITSUBST >  stack PATHS
if PRESENT :=  splitMatch(METAFACTS, { SUB | '_;_[ T1, NE:NeTermList ] })
/\ PRESENT 
----- /\ OBJECTFACTS := dt(METAFACTS)
----- [ print " splits3: " OBJECTFACTS " | PRESENT " PRESENT ]
.

crl[splits]:
    < METAFACTS | CURMAX | ONESUBST | TWOSUBST | EXISTSUBST | 
    { SUB | '_;_[ T1 ] } :: SPLITSUBST >  
    stack PATHS
=>
    < METAFACTS | CURMAX | ONESUBST | TWOSUBST | EXISTSUBST | SPLITSUBST >  stack PATHS
if PRESENT :=  splitMatch(METAFACTS, { SUB | '_;_[ T1 ] })
/\ PRESENT 
----- /\ OBJECTFACTS := dt(METAFACTS)
----- [ print " splits4:  " OBJECTFACTS " PRESENT " PRESENT ]
.
...
if RESULT  :=  join(METAFACTS, applySubst( refreshSubst( CURMAX, { SUB | T1 } ))) 
/\ NEWMAX  :=  getTerm( metaReduce(geotheory, 'maximum[ RESULT ] ))
/\ PRESENT :=  splitMatch(METAFACTS, { SUB | '_;_[ T1 ] })
/\ not PRESENT 
----- /\ OBJECTFACTS := dt(RESULT)
----- [ print " splits5: " OBJECTFACTS " | PRESENT " PRESENT ]
.

crl[goal]:
    < METAFACTS | CURMAX | ONESUBST | TWOSUBST | EXISTSUBST | SPLITSUBST >  stack  void
=>
    valid
if goalMatch( METAFACTS )
----- /\ OBJECTFACTS := dt(METAFACTS)
----- [ print " goal1:   " OBJECTFACTS ]
.

crl[goal]:
    < METAFACTS | CURMAX | ONESUBST | TWOSUBST | EXISTSUBST | SPLITSUBST >  stack  BRANCH PATHS
=>
    BRANCH stack PATHS
if goalMatch( METAFACTS )
----- /\ OBJECTFACTS := dt(METAFACTS)
----- [ print " goal2:   " OBJECTFACTS ]
.
